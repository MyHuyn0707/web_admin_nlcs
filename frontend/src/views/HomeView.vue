            ...
            <v-card>
              ...
              autocomplete="off">
                ...
                <v-card-actions>
                  ...
                  <v-btn color="light-blue darken-4" dark type="submit" outlined small>
            ...
            <v-card class="mx-auto elevation-19">
              ...
              <v-data-table 
                ...
                <template v-slot:item.actions="{item}">
                  <v-btn color="green" class="mx-2" x-small @click="editPost(item)" outlined>
                    <v-icon small>mdi-pencil</v-icon>
                  </v-btn>
                  <v-btn color="pink" class="mx-2" x-small @click="deletePost(item)" outlined>
                    <v-icon small>mdi-delete</v-icon>
                  </v-btn>
                </template>
                <template s-slot:no-results>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      {text: 'Action', value: 'actions', sortable: false, width: '10px'}
    ],
...
loadPosts: async function() {
  ...
  axios.get(apiURL).then(res =>{
    ...
    // Đơn giản chỉ cần gán lại cho đúng postData.image
    ...
    close() {
      ...
      setTimeout(() => {
        ...
        this.selectedImage = null; // ✅ reset ảnh mới
        this.editedIndex = -1;
      }, 300);
    },
    ...
    createPost() {
      ...
      formData.append('image', this.postData.image); // là File object
      ...
      axios.post(apiURL, formData, {
        ...
        this.postData = { name: '', description: '', image: null };
        this.close();
        this.color = 'success';
        this.text = "Post has been successfully saved";
        this.snackbar = true;
    ...
    updatePost() {
      ...
      if (this.selectedImage) {
        formData.append('image', this.selectedImage); // ảnh mới (File)
      } else {
        formData.append('image', this.postData.image); // ảnh cũ (string filename)
      }
